...
JSON: {"moves":[[ref,x,y,rot],...],
       ...
       "vias":[[net,x,y],...] or [[net,x,y,diameter],...],
       "via_diameter": 0.6}
A via without its own diameter takes "via_diameter", which defaults to 0.6 mm.
...
def plan_vias(plan):
    """Planned vias; an entry is [net, x, y] or [net, x, y, diameter]."""
    dflt = plan.get("via_diameter", VIA_DIAMETER)
    return [{'x': v[1], 'y': v[2], 'd': v[3] if len(v) > 3 else dflt, 'net': v[0]}
            for v in plan.get("vias", [])]

def main(argv):
    board, planfile = argv[1], argv[2]
    limit = float(argv[3]) if len(argv) > 3 else 0.2
